    ...
    class RequestQuery
    {
        ...
        public RequestQuery(Uri uri)
        {
            if (uri != null)
            {
                ...
                {
                    // Check if Alarms is the resource that is requested
                    if (segments[segments.Length - 1].ToLower().Trim('/') == "alarms")
                    {
                        ...
                        {
                            // Event Name
                            DataItemId = HttpUtility.ParseQueryString(uri.Query).Get("dataItemId");
